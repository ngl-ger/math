...
# Loading bay
  ...
  # qPCR data
   ...
   qpcr <- 
      read_delim("C:/Users/jjohn/OneDrive/MScthesis/data/aob.csv",",",)
...
The data preparation is finished for the AOB gene copy data. We have worked with the raw quantity values used qPCR machine (standardized on the individual runs standard curve), did summary statistics (averaging the technical replicates) and calculated gene copy number per nanogram DNA. 
...
```{r rhizo AOB group means and see if data is balanced, echo=FALSE}
...
We have an unbalanced dataset. One replicate in '[fert:no|fung:yes|time:t3]' [ID:320] did not produce a result . Moving on to a boxplot to visually inspect groups and look for outliers.

```{r rhizo AOB boxplots, echo=FALSE}
  bxp <- ggboxplot(
    genecopy.work, x = "fert", y = "qty_ng",
    color = "time", palette = "jco",
    facet.by = "fung", short.panel.labs = FALSE
  )
  bxp+ggtitle("Rhizo: AOB gene copy numbers")
```

No visual outliers. 

```{r rhizo AOB outliers, echo=FALSE}
...
All p-values above 0.05. Shapiro Test is passed but the general QQPlots has a strong tail. But is alright, because we may have for some biological groups more extreme values.
...
```{r rhizo AOB: mixed effect model  }
...
```{r rhizo AOB: mixed effect model no interaction}  
...
```{r rhizo AOB: mixed effect model no interaction no fung} 
...
```
Both main effects are non sign.
...
```{r soil 16s boxplots, echo=FALSE}
  ...
  bxp + ggtitle("Soil: AOB gene copy numbers ")
```  
...
```{r soil AOB normality test and qq plots, echo=FALSE}
# Normality test with all outliers (Shapiro Test)
  genecopy.work %>%
    group_by(fert, fung, time) %>%
    shapiro_test(qty_ng)
...
```{r soil AOB anova full model, echo = FALSE}
# Fitting a linear mixed effect model to our genecopynumber dataset
  # Treatment variables (fert|fung) are treated as fixed effects
  # Time and Block are random effect variables (can't be replicated)
  # We use the package nlme to effectively input random effect variables for our model
...
```{r soil AOB anova reduced model(both main effects), echo = FALSE}
