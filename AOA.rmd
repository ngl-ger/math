...
# Loading bay
  ...
  # qPCR data
   ...
   qpcr <- 
      read_delim("D://OneDrive/MScthesis/data/aoa.csv",",",)

  # DNA concentration

    dna <- 
      read_delim("D:/OneDrive/MScthesis/data/dna.csv",",",)
    
    trt <-
      read_delim("D:/OneDrive/MScthesis/data/treatment_factors.csv",",",) 
...
The data preparation is finished for the AOA gene copy data. We have worked with the raw quantity values used qPCR machine (standardized on the individual runs standard curve), did summary statistics (averaging the technical replicates) and calculated gene copy number per nanogram DNA. 
...
```{r rhizo AOA group means and see if data is balanced, echo=FALSE}
...
```{r rhizo AOA boxplots, echo=FALSE}
  bxp <- ggboxplot(
    genecopy.work, x = "fert", y = "qty_ng",
    color = "time", palette = "jco",
    facet.by = "fung", short.panel.labs = FALSE
  )
  bxp+ggtitle("Rhizo: AOA gene copy numbers")
```
...
```{r rhizo AOA outliers, echo=FALSE} 
...
All p-values above 0.05. We can proceed without filtering any replicates. 
...
```{r rhizo AOA: mixed effect model  }
...
```{r soil 16s boxplots, echo=FALSE}
  ...
  bxp + ggtitle("Soil: AOA gene copy numbers ")
```  
...
```{r [fert:no|fung:yes|time:t2], echo = FALSE}
inspect<- genecopy.work %>%
  filter(fert =="no" & fung == "yes" & time == "t2")
inspect
rm(inspect)

genecopy2 <- genecopy.work %>%
  filter(id != "226" & id !="263")
inspect
rm(inspect)
``` 
372 is relatively low compared to the others, but subjectively is within range. So I'll keep it for the time being

###### Datatable from group [fert:yes|fung:yes|time:t3]
```{r [fert:yes|fung:yes|time:t3], echo = FALSE}
inspect<- genecopy.work %>%
...
```{r soil AOA normality test and qq plots, echo=FALSE}
# Normality test with all outliers (Shapiro Test)
  genecopy.work %>%
    group_by(fert, fung, time) %>%
    shapiro_test(qty_ng)
...
```{r soil AOA anova full model, echo = FALSE}
# Fitting a linear mixed effect model to our genecopynumber dataset
  # Treatment variables (fert|fung) are treated as fixed effects
  # Time and Block are random effect variables (can't be replicated)
  # We use the package nlme to effectively input random effect variables for our model
...
```{r soil AOA anova reduced model(both main effects), echo = FALSE}
...
```
```{r}
fit <- lmer(qty_ng ~ fert*fung+
            (1|time),
            data=genecopy)
summary(fit)


fm1 <- lmer(qty_ng ~ (1|block) + fert*fung + time +  
             (1+time|block),
              data=genecopy.work)

fm2 <- lmer(qty_ng ~ (1+time|treatment) + fert*fung  
             ,
              data=genecopy.work)
summary(fm2)

fit <- lmer(qty_ng ~ time * treatment + 
            (time | block:subject) +
            (time * treatment | block), 
            data=genecopy)


fit1 <- lmer(qty_ng ~ fert*fung  + (0+time|block), data=genecopy.work)
fit2 <- lmer(qty_ng ~  time+ (1|block)+ (time||block), data=genecopy.work)
summary(fit2)
fit3 <- lmer(qty_ng ~ fert*fung +  (1|time/block), data=genecopy2)
summary(fit3)

fit4 <- lmer(qty_ng ~ time* fert*(fung|block) + (1|block), data=genecopy2)
summary(fit4)

genecopy.work$treatment <- as.factor(genecopy.work$treatment)
```
